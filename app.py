...
def show_jixiong_left_right(ji_list, xiong_list, birth_year):
...
def is_strict_double_he(gz1, gz2):
    # 双合判断：年/月/日/时柱与目标大运是否同时满足天干合且地支合
    if not gz1 or not gz2 or len(gz1) < 2 or len(gz2) < 2:
        return False
    gan1, dz1 = gz1[0], gz1[1]
    gan2, dz2 = gz2[0], gz2[1]
    return gan_he.get(gan1) == gan2 and zhi_he.get(dz1) == dz2

def is_strict_double_chong(gz1, gz2):
    # 双冲判断：年/月/日/时柱与目标大运是否同时满足地支冲（天干不用判断）
    if not gz1 or not gz2 or len(gz1) < 2 or len(gz2) < 2:
        return False
    dz1, dz2 = gz1[1], gz2[1]
    return dz_chong.get(dz1) == dz2
...
def calc_qiyun_age_by_terms(birth_date, gender, year_gan, solar_terms_dates):
    ...
    # 计算大运起算年龄（按3天算1岁，120天为1个大运）
    start_age = delta_days / 3
    return start_age, forward

def generate_dayun_with_years(dayun_list, start_year, start_age):
    # 每个大运10年
    result = []
    for i, gz in enumerate(dayun_list):
        y = start_year + int(start_age) + i*10
        result.append((gz, y))
    return result
...
def main():
    ...
    # 八字排盘计算
    year_gz, real_year = year_ganzhi(birth_date.year, birth_date.month, birth_date.day, birth_hour, birth_minute)
    month_branch = get_month_branch(birth_date.year, birth_date.month, birth_date.day)
    month_gz = month_stem_by_fihu_dun(year_gz[0], month_branch)
    day_gz = day_ganzhi_by_anchor(birth_date.year, birth_date.month, birth_date.day, birth_hour)
    time_gz = time_ganzhi_by_rule(day_gz, birth_hour, birth_minute)

    # 显示选择信息及四柱八字（同一行）
    st.markdown(f"""
    <div style='display:flex;align-items:center;gap:30px;margin-bottom:20px;font-size:18px;'>
        <div><b>请选择：</b>{gender}</div>
        <div><b>阳历生日：</b>{birth_date} {birth_hour:02d}:{birth_minute:02d}</div>
        <div><b>四柱八字：</b>{year_gz}  {month_gz}  {day_gz}  {time_gz}</div>
    </div>
    """, unsafe_allow_html=True)

    # 渲染四柱八字(上下两行大字体)
    render_four_pillars_two_rows(year_gz, month_gz, day_gz, time_gz)

    # 计算吉凶
    ji_list, xiong_list = analyze_bazi(year_gz, month_gz, day_gz, time_gz)

    # 计算大运起运
    # 这里用固定24节气简化示例，节气日期以每年2月4日立春为代表
    solar_terms_dates = [
        datetime.datetime(birth_date.year,2,4),
        datetime.datetime(birth_date.year,4,5),
        datetime.datetime(birth_date.year,6,6),
        datetime.datetime(birth_date.year,8,7),
        datetime.datetime(birth_date.year,10,8),
        datetime.datetime(birth_date.year,12,7),
    ]
    start_age, forward = calc_qiyun_age_by_terms(birth_date, gender, year_gz[0], solar_terms_dates)

    dayun_list = generate_dayun_list(year_gz[0], gender, month_gz, forward=forward, steps=8)

    start_year = birth_date.year + int(start_age)

    dayun_with_years = generate_dayun_with_years(dayun_list, start_year, 0)

    # 大运排盘展示
    st.subheader("大运排盘")
    col_dayun = st.container()
    st.markdown("""
    <style>
    .dayun-block {
    ...
    dayun_html = "<div style='display:flex;flex-wrap:wrap;gap:12px;'>"
    # 计算双合、双冲以年柱、月柱、日柱、时柱同时判断规则
    main_pillars = [year_gz, month_gz, day_gz, time_gz]

    for gz, y in dayun_with_years:
        # 双合判断，只要与任一柱同时满足双合，视为双合
        is_he = any(is_strict_double_he(p, gz) for p in main_pillars)
        is_ch = any(is_strict_double_chong(p, gz) for p in main_pillars)
        ...
        dayun_html += f"<div class='dayun-block {cls}' title='大运{gz}，年份{y}年'>{gz}<br>{y}年<br><span style='font-size:16px;font-weight:700;'>{label}</span></div>"

    dayun_html += "</div>"
    st.markdown(dayun_html, unsafe_allow_html=True)

    # 吉凶流年左右两栏展示
    show_jixiong_left_right(ji_list, xiong_list, birth_date.year)
